 ...
 *
 * The idea in one line: send a hash out through the browser, keep the original secret on the device, then reveal the secret over the back channel to prove you started the flow.
 *
 * An attacker who intercepts the authorization code never saw the secret, so the code is useless to them.
 *
 */
object PKCE {
    ...
    /**
     *
     * `URL_SAFE` gives the base64url alphabet, `NO_PADDING` strips `=`, and `NO_WRAP` keeps it on one line.
     *
     * All three matter: the result goes into a URL, and the server will compute its own encoding and compare strings.
     *
     */
     ...
     *
     * [SecureRandom] rather than [kotlin.random.Random]: these values are security tokens, and a predictable one defeats the protection entirely.
     *
     */
